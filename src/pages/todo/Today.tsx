...
const Today = () => {
      ...
      <div key={item.id} className="relative">
        <div className="relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex">
            ...
            >
              <button
                onClick={() => handleSwipeAction(() => updateItem(item.id, { completed: !item.completed }))}
              ...
              <button
                onClick={() => handleSwipeAction(() => updateItem(item.id, { isPinned: !item.isPinned }))}
          ...
          {isSelectionMode && (
            ...
            <Checkbox
              ...
              style={{ borderColor: item.completed ? undefined : getPriorityColor(item.priority || 'none') }}
            />
            <div className="flex-1 min-w-0" onClick={() => !currentSwipe?.isSwiping && setSelectedTask(item)}>
              ...
              {item.voiceRecording ? (
                ...
                <div className="flex items-center gap-2">
                  {item.isPinned && <Pin className={cn(compactMode ? "h-3 w-3" : "h-3.5 w-3.5", "text-amber-500 fill-amber-500 flex-shrink-0")} />}
  ...
  return (
    <TodoLayout title="Npd" searchValue={viewModeSearch} onSearchChange={setViewModeSearch}>
      ...
      <Button onClick={async () => { try { await Haptics.impact({ style: ImpactStyle.Heavy }); } catch {} setIsInputOpen(true); }} className="fixed left-4 right-4 z-30 h-12 text-base font-semibold" style={{ bottom: 'calc(5rem + env(safe-area-inset-bottom, 0px))' }} size="lg">

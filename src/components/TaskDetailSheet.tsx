...
export const TaskDetailSheet = ({ isOpen, task, onClose, onUpdate, onDelete, onDuplicate }: TaskDetailSheetProps) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast({
      title: t('tasks.attachments.added', 'Files attached'),
      description: `${newAttachments.length} ${t('tasks.attachments.filesAdded', 'file(s) added')}`,
    });
  ...
  const handleRemoveAttachment = async (attachmentId: string) => {
    ...
    toast({
      title: t('tasks.attachments.removed', 'File removed'),
    });
  };
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed inset-0 bg-card z-50 transition-all duration-500 ease-out overflow-hidden flex flex-col",
        ...
        <div className="px-5 pt-6 pb-4 border-b border-border">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2">
                  ...
                  {task.priority && task.priority !== 'none' && (
                    <div className="flex items-center gap-1.5">
                      <Flag className="h-4 w-4" style={{ color: getPriorityColor(task.priority) }} />
          ...
          <div className="flex items-center gap-4 mt-4">
            ...
            <button 
              ...
              className={cn("p-2 hover:bg-muted rounded-lg transition-colors", task.attachments?.length && "bg-sky-50 dark:bg-sky-950/20")}
            >
              <Paperclip className={cn("h-5 w-5", task.attachments?.length ? "text-sky-500" : "text-muted-foreground")} />
            </button>
